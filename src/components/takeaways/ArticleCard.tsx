...
// Slots, in order: featured image, category label, title, short intro, author.
    ...
    >
      <div className={`relative overflow-hidden ${large ? 'h-72 md:h-96' : 'h-56'}`}>
        {article.coverUrl ? (
          ...
          // Image slot kept even before a cover is uploaded, so the grid stays even.
          <div className="absolute inset-0 bg-gradient-to-br from-villa-green/85 via-villa-green to-villa-dark" />
        )}
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <div className="mt-auto flex flex-wrap items-center justify-between gap-3 pt-5">
          <span className="text-xs uppercase tracking-[0.16em] text-villa-muted">
            {byLabel} {article.author}
          </span>
          <span className="inline-flex items-center gap-1 text-sm font-medium text-villa-green">
            {readLabel}
